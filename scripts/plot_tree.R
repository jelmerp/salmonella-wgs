...
tree_file <- "results/ksnp3/with_refs/tree.core.tre"
  
## Define output files
tree_plotfile <- "results/ksnp3/tree.png"
...
ggtree(tree, layout = "rectangular") %<+% meta +    # or layout = "circular"
  geom_tiplab(aes(color = serotype), size = 2.5, offset = 0.001) +
  theme_tree2() +
  geom_rootedge(rootedge = 0.005) +
  #scale_color_manual(values = sero_cols) +
  theme(plot.margin = margin(0.2, 2, 0.2, 0.2, "cm"),
        legend.position = "left") +
  coord_cartesian(clip = "off") +
  annotate("text", x = 0.66, y = nseqs + 2, label = "Bla") +
  annotate("text", x = 0.75, y = nseqs + 2, label = "Bla2") +
  geom_text2(aes(label = label, subset = !isTip & as.numeric(label) != 100),
             hjust = 1, vjust = 1, nudge_x = -0.005,
             size = 2.5, fontface = "italic", color = "grey50")

ggsave(tree_plotfile, p)
